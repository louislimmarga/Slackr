...
def createMessageID():
    ...
    unusedMessageID = unusedMessageID + 1
    return newMessageID

def convertNonJSONable(item):
...
def searchMessageInChannel(channel, message_id):
    for message in channel['messages']:
        if(message_id == message['message_id']):
            return message
    raise ValueError(description = "message_id does not exists in the channel")
...
def channel_messages():
    ...
    #<= would make it return 51 messages, < return 50 messages
    tempChannelsMessages = {}
    while(i < endIndex):
        ...
        print(tempChannelsMessages)
        messagesToReturn['messages'].append(tempChannelsMessages)
        
        i = i + 1
    return dumps(messagesToReturn,default=convertNonJSONable)
...
def message_send():
    ...
    uEmail = decodeToken(uToken)
    userInfo = searchUserByEmail(uEmail['email'])
    checkUserLoggedIn(uToken,userInfo)
    ...
    #IF the valid react_id is only 1 , REMOVE THE BOTTOM 'LIST' to make it easier
    reacts = [{'react_id' : 1,
               'u_ids' : []}]
    ...
    save()
    return dumps({'message_id' : newMessageID})
...
def message_remove():
    ...
    if(userInfo['u_id'] != messageDetail['u_id'] and userInfo['u_id'] not in 
       currChannelInfo['owners_id'] and userInfo['permission_id'] == 3):
...
def message_react():
    ...
    if(userInfo['u_id'] in messageDetail['reacts']['u_ids']):
        raise ValueError(description = "User has reacted using this react_id")
    messageDetail['reacts']['u_ids'].append(userInfo['u_id'])
    return dumps({})
...
def message_unreact():
    ...
    if(reactID != 1):
        raise ValueError(description = "react_id is not valid")
    if(userInfo['u_id'] not in messageDetail['reacts']['u_ids']):
        raise ValueError(description = "User has not reacted using this react_id")
    messageDetail['reacts']['u_ids'].remove(userInfo['u_id'])
    return dumps({})
...
def user_profile():
    ...
    for user in usersList['users']:
        if user['u_id'] == u_id:
            ...
            print(f"returnData in loop is {returnData}")
            break
...
def search():
    ...
    for channel in userInfo['joinedChannel_id']:
        tmp = searchChannelInChannelsMessages(channel)
        channel_list.append(tmp['channel_id'])
    
    for channel_id in channel_list:
        for channel_message in channelsMessages:
            if channel_message['channel_id'] == channel_id:
                for item in channel_message['messages']:
                    message = item['message']
                    queryFound = message.find(query)
                    if queryFound != -1:
